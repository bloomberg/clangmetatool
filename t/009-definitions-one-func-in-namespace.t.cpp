...
public:
  ...
  void postProcessing(
    ...
    std::vector<std::string> func_names_expected({"foo"});
...
TEST(use_meta_tool, factory) {
  ...
  int argc = 4;
  const char *argv[] = {
      "one-func-in-namespace", CMAKE_SOURCE_DIR
      "/t/data/009-definitions-one-func-in-namespace/one-func-in-namespace.cpp",
      "--", "-xc++"};
