...
public:
  ...
  void postProcessing(
    ...
    std::vector<std::string> def_names_expected(
        {"ClassNoUsage", "ClassWithUsage", "ClassWithUsage", "ClassWithUsage",
         "ClassWithUsage", "ClassWithUsage", "foo", "template_func_no_usage",
         "template_func_with_usage", "template_func_with_usage"});
...
TEST(use_meta_tool, factory) {
  ...
  int argc = 4;
  const char *argv[] = {"two-class-defs-with-funcs", CMAKE_SOURCE_DIR
                        "/t/data/011-definitions-templates/templates.cpp",
                        "--", "-std=c++11", "-xc++"};
