...
public:
  ...
  void postProcessing(
    ...
    std::vector<std::string> def_names_expected(
        {"Bar", "Bar", "Foo", "initialized_global_var", "member_func_of_foo",
         "uninitialized_global_var", "uninitialized_static_class_var_of_foo"});
...
TEST(use_meta_tool, factory) {
  ...
  const char *argv[] = {"two-class-defs-with-funcs", CMAKE_SOURCE_DIR
                        "/t/data/010-definitions-two-class-defs-with-funcs/"
                        "two-class-defs-with-funcs.cpp",
                        "--", "-xc++"};
