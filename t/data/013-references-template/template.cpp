...
template <typename T> T called_func(T cf_arg);

template <typename T> T called_func_2(T cf2_arg) { return cf2_arg; }

int global_var_3 =
    called_func_2(1) + global_var; // 2x deps captured by varContextMatcher

int (*global_uncalled_func)(int) =
    called_func_2<int>; // captured by varContextMatcher

template <typename T> T func(T f_arg) {
  ...
  static int static_local_var =
      global_var; // captured by both varContextMatcher and funcContextMatcher

  return f_arg;
}

template <typename T>
struct Foo { // (constructor) captured by funcInRecordMatcher 2x (2
             // specializations)
  T foo_mem; // captured by fieldInRecordMatcher 3x (1 templated, 2
             // specializations)
};
