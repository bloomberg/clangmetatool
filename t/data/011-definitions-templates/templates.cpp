...
template <typename T> T template_func_with_usage(T in) { // 2 matches:
...
template <typename T> class ClassNoUsage { // CXXRecordDecl: ClassNoUsage
private:
  T CNI_dataMember;
};

template <typename T> class ClassWithUsage { // 5 matches:
