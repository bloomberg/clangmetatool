...
public:
  ...
  (std::map<std::string, clang::tooling::Replacements> &replacementsMap) {
    ...
    const char* expectedResult =
      "main >>>>>>>>>>>>>>>>>>>>>>>>>>\n"
      "  ** v1\n"
      "    - 4:3 'hello, world!' (Changed by code)\n"
      "    - 6:9 '<UNRESOLVED>' (Changed by code)\n"
      "    - 8:3 '#amazingAssignment' (Changed by code)\n"
      "    - 10:9 '<UNRESOLVED>' (Changed by code)\n"
      "main <<<<<<<<<<<<<<<<<<<<<<<<<<\n";
...
TEST(propagation_ConstantCStringPropagation, basic) {
  ...
  const char* argv[] = {
    "foo",
    CMAKE_SOURCE_DIR "/t/data/015-basic-cstring-propagation/main.c",
    "--",
    "-xc"
  };
