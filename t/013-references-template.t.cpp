...
public:
  ...
  void postProcessing(
    ...
    std::vector<std::pair<std::string, std::string>> ref_names_expected(
        {std::make_pair("Foo", "Foo"), std::make_pair("Foo", "Foo"),
         std::make_pair("Foo", "func2"), std::make_pair("Foo", "global_foo"),
         ...
         std::make_pair("foo_mem", "Foo"), std::make_pair("foo_mem", "Foo"),
    ...
    std::vector<std::pair<std::string, std::string>> dep_names_expected(
        {std::make_pair("Bar", "bar_func"), std::make_pair("Bar", "f"),
         std::make_pair("Foo", "Foo"), std::make_pair("Foo", "Foo"),
         std::make_pair("Foo", "foo_mem"), std::make_pair("Foo", "foo_mem"),
...
TEST(use_meta_tool, factory) {
  ...
  int argc = 4;
  const char *argv[] = {"references-template", CMAKE_SOURCE_DIR
                        "/t/data/013-references-template/template.cpp",
                        "--", "-xc++"};
