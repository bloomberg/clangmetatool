...
std::string get_newcode_for_ye(int64_t feature_id, int64_t value) {
  ...
  newcode_ss << value << " /* removed ye_olde_feature_toggle " << feature_id
             << " */";
...
public:
  MyTool(clang::CompilerInstance *_ci, clang::ast_matchers::MatchFinder *f)
      : ig(_ci, f), fc(_ci, f, "ye_olde_feature_toggle_is_enabled"), ci(_ci) {}
  void postProcessing(
    ...
    // accumulate all the calls, the argument, and its optional determinstic
    // value
    typedef std::tuple<const clang::CallExpr *, int64_t, std::optional<int64_t>>
        ye_call;
