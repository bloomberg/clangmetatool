...
struct BlockData {
  ...
  BlockData(unsigned i, unsigned l, bool o) : index(i), lowlink(l), onStack(o) {}
};
...
/**
 * Implement Tarjan's strongly connected components algorithm to detect strongly connected blocks
...
private:
  ...
  void strongConnect
  (unsigned& index, Stack& stack, BlockDataMap& blockData, unsigned& component, const clang::CFGBlock* v) {
...
public:
  ...
  bool inALoop(const clang::CFGBlock* b1, const clang::CFGBlock* b2) const {
    ...
    return componentMap.end() != it1
        && componentMap.end() != it2
...
StronglyConnectedBlocks::~StronglyConnectedBlocks() {
  delete impl;
}
...
bool StronglyConnectedBlocks::inALoop(const clang::CFGBlock* b1, const clang::CFGBlock* b2) const {
